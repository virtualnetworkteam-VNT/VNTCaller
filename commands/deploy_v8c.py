...
def gh_save(R):
    try:
        ...
        data={"message":"v8c result","content":base64.b64encode(json.dumps({"ts":TSF,"results":R}).encode()).decode()}
        if sha:data["sha"]=sha
...
print(TSF,"DEPLOY v8c FINAL");R={}
fd=gh_api("dashboard_v8c.html")
...
open("/tmp/dash_v8c.html","wb").write(html)
open(MSI+"/dashboard.html","wb").write(html)
ok=scp_p1("/tmp/dash_v8c.html","/tmp/dash_v8c.html")
push=rx1("pct push 108 /tmp/dash_v8c.html "+WEB+"/dashboard.html",t=20)
...
try:open(MP,"a").write("\n### v8c ["+TSF+"]\nAPI URL=MSI proxy http://192.168.10.96:8888/api\n")
except:pass
gh_save(R)
