...
def gh_save(R):
    try:
        ...
        data={"message":"v9 result","content":base64.b64encode(json.dumps({"ts":TSF,"results":R}).encode()).decode()}
        if sha:data["sha"]=sha
...
print(TSF,"DEPLOY v9");R={}
fd=gh_api("dashboard_v9.html")
raw=base64.b64decode(fd["content"].replace("\n","")).decode()
groq_key=""
try:cfg=json.load(open(CFG));groq_key=cfg.get("groq_key","")
except:pass
if not groq_key:
    try:mp=open(MP).read();m=re.search(r"gsk_[A-Za-z0-9]{40,}",mp);groq_key=m.group(0) if m else ""
    except:pass
if groq_key:raw=raw.replace("GROQ_PH",groq_key)
for old,new in [("PW2","App159earance.VnT"),("PW3","0568116899"),("PW1","116899")]:raw=raw.replace(old,new)
html=raw.encode()
print(f"HTML: {len(html)}b")
open("/tmp/dash_v9.html","wb").write(html);open(MSI+"/dashboard.html","wb").write(html)
ok=scp_p1("/tmp/dash_v9.html","/tmp/dash_v9.html")
push=rx1("pct push 108 /tmp/dash_v9.html "+WEB+"/dashboard.html",t=20)
sz=rx1("pct exec 108 -- stat -c %s "+WEB+"/dashboard.html 2>/dev/null")
print(f"CT108: {sz}b push={push or 'OK'}");R["dashboard"]=f"PASS {sz}b" if int(sz or 0)>100000 else f"FAIL {sz}b"
...
if os.path.exists(MSI+"/portal_server.py"):
    ...
    time.sleep(5)
    try:
        with urllib.request.urlopen("http://127.0.0.1:8888/api/health",timeout=5) as r2:R["msi_api"]="PASS "+str(r2.status)
    except Exception as e:
        plog=open("/tmp/portal.log").read()[-200:] if os.path.exists("/tmp/portal.log") else ""
        R["msi_api"]="FAIL:"+str(e)[:30];print("Portal log:",plog)
...
try:open(MP,"a").write("\n### v9 ["+TSF+"]\nSVG hierarchy + prompt auto-select\n")
except:pass
gh_save(R)
