import subprocess,datetime,json,urllib.request,time,re,base64
...
def gh_save(R):
    try:
        ...
        data={"message":"v8 result","content":base64.b64encode(json.dumps({"ts":TSF,"results":R}).encode()).decode()}
        if sha: data["sha"]=sha
...
file_data=json.loads(urllib.request.urlopen(urllib.request.Request(f"https://api.github.com/repos/{REPO}/contents/dashboard_v8.html",headers=hdr),timeout=30).read())
raw=base64.b64decode(file_data["content"].replace("\n","")).decode()

groq_key=""
try:
    cfg=json.load(open(CFG)); groq_key=cfg.get("groq_key","")
except: pass
if not groq_key:
    try:
        mp=open(MP).read(); m=re.search(r"gsk_[A-Za-z0-9]{40,}",mp)
        if m: groq_key=m.group(0)
    except: pass

if groq_key: raw=raw.replace("GROQ_PLACEHOLDER",groq_key)
for old,new in [("'ryan':       {pass:'***',","'ryan':       {pass:'116899',"),("'kraheelw@vntworld.com':{pass:'***',","'kraheelw@vntworld.com':{pass:'116899',"),("'khawaja':    {pass:'***',","'khawaja':    {pass:'App159earance.VnT',"),("'administrator':{pass:'***',","'administrator':{pass:'0568116899',")]:
    raw=raw.replace(old,new)

html=raw.encode()
print(f"HTML: {len(html)}b")
open("/tmp/dash_v8.html","wb").write(html)
open(MSI+"/dashboard.html","wb").write(html)
ok=scp_p1("/tmp/dash_v8.html","/tmp/dash_v8.html")
push=rx1("pct push 108 /tmp/dash_v8.html "+WEB+"/dashboard.html",t=20)
