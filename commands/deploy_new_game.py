...
game = open("/home/k/vnt-web/hannahbird.html").read() if os.path.exists("/home/k/vnt-web/hannahbird.html") else ""

# Write complete new game with pigeon + fullscreen + all features
new_game = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
...
#logo{font-size:clamp(16px,4.5vw,22px);font-weight:900;color:#FFE66D}#logo b{color:#90EE90}
...
.sn{font-size:clamp(15px,4.5vw,20px);font-weight:900;color:#FFE66D}.sl{font-size:clamp(8px,2vw,10px);color:rgba(255,255,255,0.65);text-transform:uppercase}
...
.p{background:rgba(255,255,255,0.07);border:1.5px solid rgba(255,255,255,0.18);border-radius:18px;padding:clamp(5px,1.5vw,8px) clamp(10px,3vw,14px);cursor:pointer;font-size:clamp(12px,3.2vw,14px);color:#ccc;transition:all .1s;white-space:nowrap}
...
.gbtn2{background:rgba(255,255,255,0.08);color:#888;border:1.5px solid rgba(255,255,255,0.15);border-radius:50px;padding:clamp(8px,2.5vw,11px) clamp(16px,5vw,22px);font-size:clamp(12px,3.5vw,14px);font-weight:700;cursor:default;margin:4px}
...
#lbadge{position:fixed;top:max(env(safe-area-inset-top,46px),46px);right:10px;background:rgba(0,0,0,.6);border-radius:7px;padding:3px 9px;font-size:clamp(10px,2.5vw,12px);font-weight:700;color:#FFE66D;z-index:10;display:none;border:1.5px solid #FFE66D55}
...
<div id="ov">
  ...
  <div id="olc">
    ...
    <div style="display:flex;gap:14px;flex-wrap:wrap;justify-content:center;margin-top:6px">
      <div><span class="lbl">Sky theme</span>
        <div class="row" id="tp">
          ...
          <div class="p" data-v="day">Day</div>
    ...
    <div style="color:#444;font-size:10px;text-align:center;margin-top:8px">&#x267E;&#xFE0F; Unlimited Levels &#183; VNT World AI Division</div>
  </div>
</div>
<script>
...
function rs(){
  ...
  COLS=W>H?Math.min(14,Math.max(10,Math.floor(W/(minDim/10)))):Math.min(10,Math.max(8,Math.floor(W/(minDim/9))));
  CELL=Math.min(Math.floor((W-6)/COLS),Math.floor((H-70)/ROWS));
  CELL=Math.max(CELL,Math.floor(minDim/11));
  OX=Math.floor((W-CELL*COLS)/2);OY=Math.floor((H-CELL*ROWS)/2)+8;
}
...
function playFX(t){
  if(!musicOn)return;const ac=getAC();if(!ac)return;
  const s={nest:[660,880,.18,.18],win:[523,1046,.28,.5],lose:[440,220,.22,.4],speed:[880,1200,.18,.15],fire:[300,150,.18,.2],area:[550,750,.22,.2],super:[440,880,.28,.4],start:[523,659,.18,.3],lvl:[660,1320,.28,.5]};
  const[f1,f2,vol,dur]=s[t]||[440,440,.13,.2];
...
function startMusic(){
  if(!musicOn||musicPlaying)return;const ac=getAC();if(!ac)return;
  musicPlaying=true;
  const mg=ac.createGain();mg.gain.value=.14;mg.connect(ac.destination);
  const ns=[261.63,293.66,329.63,392,440,523.25,587.33,659.25,783.99,880];
  const ml=[0,2,4,7,9,12,9,7,4,2,0,4,7,9,12,9,2,4,7,4,0,4,9,7,0,2,4,7,4,2,0,4].map(i=>ns[i%ns.length]);
  const beat=60/155;let t=ac.currentTime+.1;
  function loop(){
    ml.forEach((f,i)=>{const o=ac.createOscillator(),g=ac.createGain();o.type='square';o.frequency.value=f;g.gain.setValueAtTime(0,t+i*beat*.5);g.gain.linearRampToValueAtTime(.32,t+i*beat*.5+.02);g.gain.exponentialRampToValueAtTime(.001,t+i*beat*.5+beat*.42);o.connect(g);g.connect(mg);o.start(t+i*beat*.5);o.stop(t+i*beat*.5+beat*.5);});
    [65.41,73.42,82.41,87.31,65.41,82.41,87.31,73.42].forEach((f,i)=>{const o=ac.createOscillator(),g=ac.createGain();o.type='sawtooth';o.frequency.value=f;g.gain.setValueAtTime(0,t+i*beat*2);g.gain.linearRampToValueAtTime(.18,t+i*beat*2+.05);g.gain.exponentialRampToValueAtTime(.001,t+i*beat*2+beat*1.8);o.connect(g);g.connect(mg);o.start(t+i*beat*2);o.stop(t+i*beat*2+beat*2);});
    const ll=ml.length*beat*.5;t+=ll;if(musicPlaying)setTimeout(loop,(ll-.4)*1000);
  }
  loop();
}
...
function cap(r,c,t,sp=true){
  if(r<0||r>=ROWS||c<0||c>=COLS)return;
  grid[r][c].owner=t;
  if(grid[r][c].pw&&t==='player'){apw(grid[r][c].pw,r,c);grid[r][c].pw=null;}
  if(t==='player'){score+=combo>3?2:1;lvlScore++;combo++;}
  if(sp&&grid[r][c].nest){
    spPts(cx(c),cy(r),getFC(t));addF(t==='player'?'\uD83C\uDFE0'+(combo>3?'+2':'+1'):'\uD83C\uDFE0',cx(c),cy(r));
    if(t==='player'&&musicOn)playFX('nest');
    const sr=t==='player'?(pwA.area>Date.now()?3:2):1,bn=diff==='easy'?.2:0;
    for(let dr=-sr;dr<=sr;dr++)for(let dc=-sr;dc<=sr;dc++){if(Math.abs(dr)+Math.abs(dc)<=sr){const nr=r+dr,nc=c+dc;if(nr>=0&&nr<ROWS&&nc>=0&&nc<COLS&&grid[nr][nc].owner!==t&&Math.random()<(t==='player'?.64+bn:.44)){grid[nr][nc].owner=t;if(t==='player'){score++;lvlScore++;}}}}
  }
}
function apw(type,r,c){pwA[type]=Date.now()+(type==='super'?12000:5000);const m={speed:'\uD83C\uDF2A Speed!',fire:'\uD83D\uDD25 Fire!',area:'\uD83D\uDCA5 Blast!',super:'\u26A1 Super!'};addF(m[type],cx(c),cy(r));spPts(cx(c),cy(r),'#FFD700');if(musicOn)playFX(type);}
...
function mv(b,dr,dc){
  if(b.frozen>Date.now())return false;
  const nr=b.r+dr,nc=b.c+dc;if(nr<0||nr>=ROWS||nc<0||nc>=COLS)return false;
  b.r=nr;b.c=nc;b.tr.push({r:nr,c:nc});if(b.tr.length>6)b.tr.shift();
  if(b.type==='player'&&pwA.fire>Date.now()){[[-1,0],[1,0],[0,-1],[0,1]].forEach(([fr,fc])=>{const er=nr+fr,ec=nc+fc;if(er>=0&&er<ROWS&&ec>=0&&ec<COLS){const tgt=enemies.find(e=>e.r===er&&e.c===ec);if(tgt){tgt.frozen=Date.now()+2500;addF('\uD83D\uDD25',cx(ec),cy(er));}}});}
  cap(nr,nc,b.type,true);return true;
}
...
function uhud(){
  const{p,e,tot}=gc();const pct=Math.round(p/tot*100);
  document.getElementById('sn').textContent=p;document.getElementById('sp').textContent=pct+'%';
  document.getElementById('sc').textContent=score;document.getElementById('se').textContent=e;
  document.getElementById('pbf').style.width=pct+'%';
  if(pct>=(65+(diff==='easy'?10:0)))nextLevel();
  else if(e>=tot*(diff==='easy'?.8:.68))endGame(false);
}
...
function endGame(win){running=false;cancelAnimationFrame(raf);stopMusic();const{p,tot}=gc();if(musicOn)playFX(win?'win':'lose');showMsg(win?'\uD83C\uDFC6':'\uD83D\uDC94',win?pName+' Won!':'So close!','Level '+level+' \u2022 Score: '+score+' \u2022 '+Math.round(p/tot*100)+'% territory',win?'\uD83D\uDD04 Play Again':'\uD83D\uDD04 Try Again',()=>{level=1;score=0;go();},true);}

function showMsg(icon,title,sub,btn,cb,home=false){
  const ov=document.getElementById('ov'),olc=document.getElementById('olc');
  ov.style.display='flex';
  document.getElementById('hud').style.display='none';
  ['bst','qbtn','lbadge','pwt','micon'].forEach(id=>{const el=document.getElementById(id);if(el)el.style.display='none';});
  olc.innerHTML='<div style="text-align:center;padding:10px"><div style="font-size:clamp(52px,14vw,72px);margin-bottom:8px">'+icon+'</div><div style="color:#FFE66D;font-size:clamp(20px,6vw,26px);font-weight:900;margin-bottom:6px">'+title+'</div><div style="color:#aaa;font-size:clamp(12px,3.5vw,15px);margin-bottom:18px">'+sub+'</div><div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap"><button class="gbtn" onclick="('+cb.toString()+')()">'+btn+'</button>'+(home?'<button class="gbtn2" onclick="location.reload()" style="cursor:pointer;opacity:1;color:#ccc">Menu</button>':'')+'</div><div style="color:#444;font-size:10px;margin-top:10px">\u267E\uFE0F Unlimited Levels</div></div>';
}

function continueGame(){
  document.getElementById('ov').style.display='none';
  document.getElementById('hud').style.display='block';
  ['bst','micon'].forEach(id=>{const el=document.getElementById(id);if(el)el.style.display='flex';});
  document.getElementById('qbtn').style.display='block';
  document.getElementById('lbadge').style.display='block';
  document.getElementById('lbadge').textContent='Lv '+level;
  document.getElementById('micon').textContent=musicOn?'\uD83D\uDD0A':'\uD83D\uDD07';
  rs();ig();lvlScore=0;combo=0;
  const ec=ecnt(),pos=[[Math.floor(ROWS/2),0],[Math.floor(ROWS/2),COLS-1],[0,Math.floor(COLS/2)],[ROWS-1,Math.floor(COLS/2)]];
  player=mb(pos[0][0],pos[0][1],'player',pBird);
  enemies=[];const ee=['\uD83E\uDD85','\uD83E\uDD89','\uD83E\uDD9A'];
  for(let i=0;i<ec;i++)enemies.push(mb(pos[i+1][0],pos[i+1][1],'e'+i,ee[i]));
  cap(player.r,player.c,'player',false);enemies.forEach((e,i)=>cap(e.r,e.c,'e'+i,false));
  pts=[];fts=[];pwA={speed:0,fire:0,area:0,super:0};lastSuper=Date.now();
  addF('Go '+pName+'!',cx(player.c),cy(player.r));
  running=true;uhud();requestAnimationFrame(tick);
  if(musicOn){getAC()?.resume?.();startMusic();}
}
...
function tick(ts){
  ...
  if(Date.now()-lastSuper>10000){lastSuper=Date.now();pwA.super=Date.now()+8000;addF('\u26A1 Supercharge!',cx(player.c),cy(player.r));spPts(cx(player.c),cy(player.r),'#FFD700');if(musicOn)playFX('super');}
  const kspd=pwA.speed>Date.now()?65:120;
  if(ts-lastKey>kspd){if(keys['ArrowUp']||keys['w']){mv(player,-1,0);combo++;uhud();lastKey=ts;}else if(keys['ArrowDown']||keys['s']){mv(player,1,0);combo++;uhud();lastKey=ts;}else if(keys['ArrowLeft']||keys['a']){mv(player,0,-1);combo++;uhud();lastKey=ts;}else if(keys['ArrowRight']||keys['d']){mv(player,0,1);combo++;uhud();lastKey=ts;}}
  const espd=Math.max(110,480-(diff==='speed'?360:diff==='hard'?200:diff==='medium'?100:0)-(level-1)*12);
  if(ts-lastEn>espd){lastEn=ts;enemies.forEach(en=>{if(en.frozen>Date.now())return;let bm=null,bs=-9;[[-1,0],[1,0],[0,-1],[0,1]].forEach(([dr,dc])=>{const nr=en.r+dr,nc=en.c+dc;if(nr<0||nr>=ROWS||nc<0||nc>=COLS)return;let sc=grid[nr][nc].owner==='player'?4:grid[nr][nc].owner==='neutral'?2:grid[nr][nc].owner===en.type?-1:1;if(grid[nr][nc].nest)sc+=3;sc+=Math.random()*(diff==='easy'?4:diff==='hard'?1:2);if(sc>bs){bs=sc;bm=[dr,dc];}});if(bm)mv(en,bm[0],bm[1]);});}
  pts=pts.filter(p=>{p.x+=p.vx;p.y+=p.vy;p.vy+=.12;p.life-=.05;return p.life>0;});fts=fts.filter(f=>{f.y+=f.vy;f.life-=.022;return f.life>0;});
  const now=Date.now(),active=Object.entries(pwA).find(([k,v])=>v>now),pt=document.getElementById('pwt');
  if(active&&pt){pt.style.display='block';pt.textContent={speed:'\uD83C\uDF2A',fire:'\uD83D\uDD25',area:'\uD83D\uDCA5',super:'\u26A1'}[active[0]]+' '+Math.ceil((active[1]-now)/1000)+'s';}else if(pt)pt.style.display='none';
  draw();
}

function draw(){
  const th=getTheme();ctx.fillStyle=th.sky;ctx.fillRect(0,0,W,H);
  if(th.stars){ctx.fillStyle='rgba(255,255,255,.8)';[[W*.1,H*.04],[W*.3,H*.025],[W*.6,H*.045],[W*.8,H*.03],[W*.45,H*.02],[W*.9,H*.04]].forEach(([x,y])=>{ctx.beginPath();ctx.arc(x,y,Math.max(1.5,W*.003),0,Math.PI*2);ctx.fill();});}
  ctx.fillStyle=th.cld;[[W*.14,H*.07,W*.12],[W*.52,H*.05,W*.14],[W*.8,H*.08,W*.1],[W*.35,H*.1,W*.08]].forEach(([x,y,s])=>{ctx.beginPath();ctx.arc(x,y,s/2,0,Math.PI*2);ctx.arc(x+s*.32,y-s*.18,s/3,0,Math.PI*2);ctx.arc(x-s*.28,y-s*.12,s/3,0,Math.PI*2);ctx.fill();});
  const FC={player:getC(),e0:'#FF8888',e1:'#88BBFF',e2:'#FFCC77',neutral:th.gnd};
  const rad=Math.max(4,CELL*.1);
  for(let r=0;r<ROWS;r++){for(let c=0;c<COLS;c++){const cell=grid[r][c],x=gx(c),y=gy(r);ctx.fillStyle=FC[cell.owner]||FC.neutral;ctx.beginPath();ctx.roundRect(x+2,y+2,CELL-4,CELL-4,rad);ctx.fill();ctx.strokeStyle='rgba(255,255,255,.2)';ctx.lineWidth=1;ctx.beginPath();ctx.roundRect(x+2,y+2,CELL-4,CELL-4,rad);ctx.stroke();const fs=Math.floor(CELL*.55)+'px serif';ctx.textAlign='center';ctx.textBaseline='middle';if(cell.tree&&cell.owner==='neutral'){ctx.font=fs;ctx.fillText(th.stars?'\uD83C\uDF32':'\uD83C\uDF33',cx(c),cy(r));}if(cell.nest){ctx.font=fs;ctx.fillText('\uD83E\uDEB9',cx(c),cy(r));}if(cell.pw){ctx.font=Math.floor(CELL*.46)+'px serif';ctx.fillText({speed:'\uD83C\uDF2A',fire:'\uD83D\uDD25',area:'\uD83D\uDCA5',super:'\u26A1'}[cell.pw],cx(c),cy(r));}}}
  if(pwA.super>Date.now()){ctx.globalAlpha=.15+.08*Math.sin(Date.now()/150);ctx.fillStyle='#FFD700';for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++){if(grid[r][c].owner==='player'){ctx.beginPath();ctx.roundRect(gx(c)+2,gy(r)+2,CELL-4,CELL-4,rad);ctx.fill();}}ctx.globalAlpha=1;}
  const pcol=getC();const rgb=pcol.startsWith('#')&&pcol.length===7?parseInt(pcol.slice(1,3),16)+','+parseInt(pcol.slice(3,5),16)+','+parseInt(pcol.slice(5,7),16):'126,255,126';
  player.tr.forEach((t,i)=>{ctx.fillStyle='rgba('+rgb+','+(i/player.tr.length*.4)+')';ctx.beginPath();ctx.roundRect(gx(t.c)+4,gy(t.r)+4,CELL-8,CELL-8,4);ctx.fill();});
  [player,...enemies].forEach(b=>{
    const bx=cx(b.c),by=cy(b.r),frz=b.frozen>Date.now();
    const bc=b.type==='player'?getC():b.type==='e0'?'#FF8888':b.type==='e1'?'#88BBFF':'#FFCC77';
    if(frz){ctx.fillStyle='rgba(100,200,255,.25)';ctx.beginPath();ctx.arc(bx,by,CELL*.58,0,Math.PI*2);ctx.fill();}
    ctx.fillStyle=bc+'50';ctx.beginPath();ctx.arc(bx,by,CELL*.52,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='rgba(0,0,0,.1)';ctx.beginPath();ctx.ellipse(bx,by+CELL*.38,CELL*.28,CELL*.09,0,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='rgba(255,255,255,.92)';ctx.beginPath();ctx.arc(bx,by,CELL*.42,0,Math.PI*2);ctx.fill();
    ctx.font=Math.floor(CELL*.6)+'px serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(b.emoji,bx,by+2);
    if(b.type==='player'){const nw=Math.max(44,pName.length*8+10);ctx.fillStyle='rgba(0,0,0,.68)';ctx.beginPath();ctx.roundRect(bx-nw/2,by+CELL*.4,nw,Math.max(13,CELL*.18),4);ctx.fill();ctx.fillStyle='#FFE66D';ctx.font='bold '+Math.max(9,Math.floor(CELL*.17))+'px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(pName,bx,by+CELL*.49+Math.max(6,CELL*.09));}
  });
  pts.forEach(p=>{ctx.globalAlpha=p.life;ctx.fillStyle=p.col;ctx.beginPath();ctx.arc(p.x,p.y,p.sz*p.life,0,Math.PI*2);ctx.fill();});ctx.globalAlpha=1;
  fts.forEach(f=>{ctx.globalAlpha=f.life;ctx.font='bold '+Math.max(12,Math.floor(CELL*.35))+'px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillStyle='rgba(0,0,0,.5)';ctx.fillText(f.t,f.x+1,f.y+1);ctx.fillStyle='#FFE66D';ctx.fillText(f.t,f.x,f.y);});ctx.globalAlpha=1;
}
...
# Write game to all locations
for path in [WEB+"/hannahbird.html", APP_WWW+"/index.html"]:
    open(path,"w").write(new_game)
    print("Written:", path)

# Update webserver to serve APK too
import subprocess, shutil, os

# Sync capacitor and build APK
print("Syncing Capacitor...")
r=subprocess.run("export JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64 && cd /home/k/hannahbird-app && npx cap sync android 2>&1 | tail -3",
...
if os.path.exists(apk):
    ...
    print("Build failed:", r2.stderr[-300:])

import datetime
open("/mnt/vnt-data/FileServer/VNT_World_AI_Division/MemPalace.md","a").write(
    "\n### HannahBird Full Game ["+datetime.datetime.now().strftime("%Y-%m-%d %H:%M")+"]\n"
    "New game with pigeon, fullscreen, music, levels, powerups\n"
    "Web: http://192.168.10.96:8888/hannahbird.html\n"
    "APK: http://192.168.10.96:8888/hannahbird.apk\n"
)
print("DONE - check Redmi for new game")
