...
def gh_save(R):
    try:
        ...
        data={"message":"v8b result","content":base64.b64encode(json.dumps({"ts":TSF,"results":R}).encode()).decode()}
        if sha: data["sha"]=sha
...
print(TSF,"DEPLOY v8b"); R={}
fd=gh_api("dashboard_v8b.html")
...
for old,new in [("USER_PW2","App159earance.VnT"),("USER_PW3","0568116899"),("USER_PW1","116899")]: raw=raw.replace(old,new)
html=raw.encode()
print(f"HTML: {len(html)}b")
open("/tmp/dash_v8b.html","wb").write(html); open(MSI+"/dashboard.html","wb").write(html)
ok=scp_p1("/tmp/dash_v8b.html","/tmp/dash_v8b.html")
push=rx1("pct push 108 /tmp/dash_v8b.html "+WEB+"/dashboard.html",t=20)
sz=rx1("pct exec 108 -- stat -c %s "+WEB+"/dashboard.html 2>/dev/null")
print(f"CT108: {sz}b"); R["dashboard"]=f"PASS {sz}b" if int(sz or 0)>90000 else f"FAIL {sz}b"
